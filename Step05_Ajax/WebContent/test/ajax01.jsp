	...
	<script>
		document.querySelector("#getBtn")
			.addEventListener("click",function(){
				//fetch() 함수를 이용해서 get_data.jsp 페이지에 GET 방식을 요청한다.
				fetch("${pageContext.request.contextPath}/test/get_data.jsp").then(function(response){
					//단순 문자열인 경우. text
		...
		.addEventListener("click",function(){
			...
			fetch("get_data3.jsp").then(function(response){			
				//JSON 문자열을 서버에서 응답 했을 때는 .json()함수를 호출해서 리턴한다.
